...
function AddMovie({ onClose }: AddMovieProps) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!title || !description || !videoUrl || !thumbnaiUrl || posterUrl|| !genre || !duration) {
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70 z-50 " >
      ...
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
        <form onSubmit={handleSubmit} className="flex flex-col space-y-4 bg-slate-100 p-8 rounded w-3/4 h-auto max-w-3xl ">
          ...
          <div>
            <input type="text" value={thumbnaiUrl} onChange={(e) => setThumbnailUrl(e.target.value)} placeholder="Image URL (vertical)" className="p-2 border rounded w-full" required pattern="https?://.+" title="Must be a valid URL" />
          </div>
          <div>
            <input type="text" value={posterUrl} onChange={(e) => setPosterUrl(e.target.value)} placeholder="Poster URL (horizontal)" className="p-2 border rounded w-full" required pattern="https?://.+" title="Must be a valid URL" />
          </div>
          <div>
